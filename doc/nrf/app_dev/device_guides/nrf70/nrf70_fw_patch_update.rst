...
See :ref:`zephyr:flash_map_api` for how partitions map to the flash map used by MCUboot and the application.
...
The DFU procedure for the nRF70 Series firmware patch, when located in external memory, requires additional MCUboot primary and secondary slot partitions on the external flash.
Define these regions in devicetree overlays or shared DTS fragments for your board or application.

The instructions in this guide assume you use devicetree-based partitioning (``compatible = "fixed-partitions"`` on the external NVM device).
...
To add the required regions for the nRF70 Series firmware patch update, define partitions under the external flash device (for example ``&mx25r64``) in devicetree overlays or shared DTS fragments.
Within a single ``fixed-partitions`` node, sibling partitions must not overlap.

1. Plan the MCUboot image slot for the Wi-Fi firmware child image: the slot begins with an MCUboot image header (512 bytes, ``0x200`` in this example), followed by 128 kB (``0x20000``) for the firmware image body.
   The full primary slot size is 132 kB + 512 B (``0x21000``), aligned to the external flash sector size (for example 4 kB on MX25R64).

#. Add a partition with nodelabel ``nrf70_wifi_fw_partition`` for the firmware image body (128 kB).
   Its base address must match where the nRF Wi-Fi build places the patch (immediately after the MCUboot header inside the primary slot, ``0x12f200`` in this example).
   Because the firmware body lies inside the same address range as the MCUboot primary slot, you cannot describe both as overlapping children of the same ``fixed-partitions`` node; follow your board's pattern (for example MCUboot partitions in one overlay and ``nrf70_wifi_fw_partition`` in the application overlay, or a single merged memory map used by sysbuild).

#. Reserve any remaining external flash with additional partition nodes as required by your product.

The following devicetree fragment shows the ``nrf70_wifi_fw_partition`` node that the nRF Wi-Fi build expects; parent device, partition offsets, and sizes must match your full external flash layout:
...
If you migrate devices from an older release, keep partition addresses and sizes identical to the previous layout.
