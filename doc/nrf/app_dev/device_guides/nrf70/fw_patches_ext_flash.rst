...
The partition node must use the nodelabel ``nrf70_wifi_fw_partition`` (see the ``nrf70-fw-patch-ext-flash`` snippet overlay).
For example:
...
Adjust the parent node (for example ``&mx25r64``), offsets, and sizes to match your board's external flash layout.
